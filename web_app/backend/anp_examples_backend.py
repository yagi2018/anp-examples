...
from web_app.backend.models import QueryRequest, QueryResponse, AgentDocTreeRequest, AgentDocTreeResponse, GetDocumentRequest, GetDocumentResponse
...
async def read_agent_doc_tree():
    """提供智能体文档树结构页面"""
    try:
        with open(BASE_DIR / "frontend" / "agent-doc-tree.html", "r", encoding="utf-8") as f:
            content = f.read()
        return content
    except Exception as e:
        logging.error(f"读取智能体文档树结构页面出错: {str(e)}")
        raise HTTPException(status_code=500, detail=f"读取智能体文档树结构页面出错: {str(e)}")
...
async def read_jsonld_viewer():
    """提供JSON-LD查看器页面"""
    try:
        with open(BASE_DIR / "frontend" / "jsonld-viewer.html", "r", encoding="utf-8") as f:
            content = f.read()
        return content
    except Exception as e:
        logging.error(f"读取JSON-LD查看器页面出错: {str(e)}")
        raise HTTPException(status_code=500, detail=f"读取JSON-LD查看器页面出错: {str(e)}")
...
async def read_jsonld_network():
    """提供JSON-LD网络页面"""
    try:
        with open(BASE_DIR / "frontend" / "jsonld-network.html", "r", encoding="utf-8") as f:
            content = f.read()
        return content
    except Exception as e:
        logging.error(f"读取JSON-LD网络页面出错: {str(e)}")
        raise HTTPException(status_code=500, detail=f"读取JSON-LD网络页面出错: {str(e)}")
...
async def agent_doc_tree(request: AgentDocTreeRequest):
    ...
    try:
        ...
        # 递归获取文档
        await crawl_doc_tree(initial_url, anp_tool, visited_urls, crawled_documents, level=0, max_level=5)
...
async def crawl_doc_tree(url, anp_tool, visited_urls, crawled_documents, level=0, max_level=5, max_docs=30):
    """递归获取文档及其链接的函数"""
    ...
    try:
        ...
        for link in links:
            ...
            await crawl_doc_tree(link, anp_tool, visited_urls, crawled_documents, level+1, max_level, max_docs)
